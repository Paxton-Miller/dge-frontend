...
const title = ref('Data Governance Environment Platform')
...
const submit = async (formValidator) => {
  if (!formValidator)
    return
  await formValidator.validate(async valid => {
    if (valid) {
      ...
      if (token) {
        ...
        const sessionWebUser = JSON.parse(toolObj.FormatToken(localStorage.token))

        const user = await getUserById(sessionWebUser.id)
...
.login-container {
  min-height: 100vh;
  width: 100%;
  background: linear-gradient(to bottom, #1a3e72, #71879a);
  //background-color: $bg;
  overflow: hidden;
